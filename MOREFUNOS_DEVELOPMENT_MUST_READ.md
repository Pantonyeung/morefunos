...
安全／資料完整／不可逆風險 → Master Authority → 本文件＋Current Registry＋Document Authority Classification＋Low Cost CI → repo `AGENTS.md`／PRIMARY STANDARD → Ownership／Decision／Current Lock／Change Impact → active branch／PR／head evidence → 最新產品明確決定 → Drive／Jade。
...
5. `MOREFUNOS_LOW_COST_CI_DEVELOPMENT_PROTOCOL_V1.0.md`
6. `MOREFUNOS_ENGINEERING_PITFALLS_AND_PROVEN_SOLUTIONS.md`
7. 對應 repo 最新 `AGENTS.md`、Lock、Change Impact、Handoff、active PR／head evidence

未讀完，禁止修改、建立新 CI、合拼或宣稱完成。

## 3. 舊文件降權硬規則

以下全部預設 `REFERENCE ONLY / NON-AUTHORITY / DO NOT IMPLEMENT DIRECTLY`：
- WORK01／WORK02／WORK03
- `TEST_WORK03_UNIFIED_LOGIN`、Session／Bootstrap／Push／Pull／Heartbeat／Fallback
- Apps Script V1.2.x／Staff Auth／Sync Runtime／舊 gateway
- 舊 Google Sheet 主資料庫／即時 Order Truth／派號／同步 Authority 文件
- V42／SA2／EG，包括 `分支 · V42EG 設計與開發.txt`
- 舊 Customer root-five-file／版本快照
- 舊 SMM independent core
- 仍引用以上架構嘅舊四端整合包、Final Lock、接手主題包

呢啲文件只可抽取背景、UI 素材、舊商業規則、migration、Contract、踩坑同測試案例；不得產生 Current Next Step、不得覆蓋現行 Architecture、不得直接指導新四端閉環施工。標題含 `LOCK`／`FINAL`／`MUST READ`／`CURRENT` 亦唔會自動提升權威。
...
### WORK03／Apps Script／Google Sheet
- WORK03 Login／Push／Pull／Heartbeat／Fallback：`SUPERSEDED / DO NOT CONTINUE`。
- Apps Script Staff Auth／Google Sheet Sync Runtime：`REFERENCE ONLY`。
- 現役架構：Firebase Auth＋Firebase RTDB＋Cloudflare Worker＋Google Sheet V2 ledger mirror。
- Google Sheet V2 不可成為即時 Order Truth、定價、正式派號、付款、Print Job 或 Runtime Authority。
...
- 從 Reference／Historical 文件直接建立施工指令；必須先 reconciliation／re-adoption。
